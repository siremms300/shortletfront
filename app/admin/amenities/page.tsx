...
const handleSubmit = async (e: React.FormEvent) => {
  ...
  try {
    ...
    console.log('📤 Sending amenity data:', amenityData);
    console.log('📤 Full request payload:', JSON.stringify(amenityData, null, 2));

    let response;
    if (editingAmenity) {
      console.log(`🔄 Updating amenity: ${editingAmenity._id}`);
      response = await amenitiesAPI.updateAmenity(editingAmenity._id, amenityData);
    } else {
      console.log('🆕 Creating new amenity');
      response = await amenitiesAPI.createAmenity(amenityData);
    }
    ...
    console.error('💥 Error saving amenity:', {
      message: err.message,
