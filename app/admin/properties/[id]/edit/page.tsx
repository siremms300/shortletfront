...
export default function EditPropertyPage({ params }: PageProps) {
  ...
  const fetchProperty = async () => {
    try {
      ...
      setFormData({
        ...
        utilityPercentage: propertyData.utilityPercentage?.toString() || '20',
        serviceChargePercentage: propertyData.serviceChargePercentage?.toString() || '10',
        vatPercentage: propertyData.vatPercentage?.toString() || '7.5',
        // Discount fields
        discountType: propertyData.discount?.type || '',
        discountValue: propertyData.discount?.value?.toString() || '',
        discountStartDate: propertyData.discount?.startDate ? new Date(propertyData.discount.startDate).toISOString().split('T')[0] : '',
        discountEndDate: propertyData.discount?.endDate ? new Date(propertyData.discount.endDate).toISOString().split('T')[0] : '',
        discountActive: propertyData.discount?.isActive || false
      });
  ...
  const calculateDiscountedPrice = () => {
    if (!formData.price || !formData.discountActive || !formData.discountValue) return null;
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="border border-gray-200 rounded-lg p-6">
            <h3 className="text-lg font-semibold text-[#383a3c] mb-4">Price Calculation Settings</h3>
          ...
          {/* Discount Section */}
          <div className="border border-gray-200 rounded-lg p-6">
            <h3 className="text-lg font-semibold text-[#383a3c] mb-4">Discount Settings</h3>
            ...
            <div className="flex items-center mb-4">
              <input
                ...
                onChange={(e) => setFormData(prev => ({ ...prev, discountActive: e.target.checked }))}
                className="rounded border-gray-300 text-[#f06123] focus:ring-[#f06123] mr-2 h-5 w-5"
              />
              <label htmlFor="discountActive" className="text-base font-medium text-gray-700">
                Enable Discount for this Property
              </label>
            </div>
            ...
            {formData.price && formData.discountActive && formData.discountValue && (
              <div className="mt-4 p-4 bg-blue-50 rounded-lg">
                <p className="text-sm text-blue-800">
                  <span className="font-medium">Preview:</span> Original Price: ₦{parseFloat(formData.price).toLocaleString()}
                  {formData.discountType === 'percentage' && (
                    <>
                      <br />→ Discounted Price: ₦{calculateDiscountedPrice()?.toLocaleString()} 
                  ...
                  {formData.discountType === 'fixed' && (
                    <>
                      <br />→ Discounted Price: ₦{calculateDiscountedPrice()?.toLocaleString()} 
                      <span className="ml-2 bg-green-100 text-green-800 px-2 py-0.5 rounded-full text-xs">
                        Save ₦{parseFloat(formData.discountValue).toLocaleString()}
                      </span>
                    </>
                  )}
                </p>
                {formData.discountStartDate && (
                  <p className="text-xs text-blue-600 mt-1">
                    Valid from {new Date(formData.discountStartDate).toLocaleDateString()}
                    {formData.discountEndDate && ` to ${new Date(formData.discountEndDate).toLocaleDateString()}`}
                  </p>
                )}
              </div>
            )}
