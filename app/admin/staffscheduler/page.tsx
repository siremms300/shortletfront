...
const DEFAULT_SCHEDULE: WorkSchedule = {
  ...
  saturday: { start: '08:00', end: '14:00', working: true }, // Half day Saturday
  sunday: { start: '10:00', end: '16:00', working: false } // Flexible Sunday (default off)
};
...
export default function StaffScheduler() {
  ...
  const [editingSchedule, setEditingSchedule] = useState<WorkSchedule>(DEFAULT_SCHEDULE);
  ...
  const isStaffScheduledToday = (staff: StaffMember) => {
    ...
    return staff.schedule?.[days[today] as keyof WorkSchedule]?.working || false;
  };
  ...
  const updateStaffSchedule = async () => {
    ...
    try {
      const response = await staffAPI.updateStaff(selectedStaff._id, {
        schedule: editingSchedule
      });
      
      setStaff(staff.map(s => 
        s._id === selectedStaff._id ? response.staff : s
      ));
  ...
  const openScheduleEditor = (staff: StaffMember) => {
    setSelectedStaff(staff);
    setEditingSchedule(staff.schedule || DEFAULT_SCHEDULE);
    setShowScheduleModal(true);
  };

  const updateScheduleDay = (day: string, field: 'start' | 'end' | 'working', value: string | boolean) => {
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="text-sm text-gray-500">
              Default: Mon-Fri 8am-5pm | Sat 8am-2pm | Sun Flexible
            </div>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  {DAYS.map(day => (
                    <th key={day.key} className="px-4 py-2 text-center text-xs font-medium text-gray-500 uppercase">
                      {day.label}
                    </th>
                  ))}
              ...
              <tbody className="divide-y divide-gray-200">
                {staff.map((member) => (
                  <tr key={member._id}>
                    ...
                    {DAYS.map((day) => {
                      const schedule = member.schedule?.[day.key as keyof WorkSchedule] || DEFAULT_SCHEDULE[day.key as keyof WorkSchedule];
                      return (
                        <td key={day.key} className="px-4 py-2 text-center">
                          {schedule.working ? (
                            <div className="text-xs">
                              <span className="text-green-600 font-medium">{schedule.start}</span>
                              <span className="text-gray-400 mx-1">-</span>
                              <span className="text-red-600 font-medium">{schedule.end}</span>
                            </div>
                          ) : (
                    ...
                    <td className="px-4 py-2 text-center">
                      <button
                        onClick={() => openScheduleEditor(member)}
          ...
          <div className="bg-white rounded-2xl p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <div className="pt-4 border-t border-gray-200">
                <h4 className="font-medium text-gray-700 mb-3">Default Schedule (Can be edited later)</h4>
                <p className="text-sm text-gray-500 mb-3">Mon-Fri: 8am-5pm | Sat: 8am-2pm | Sun: Flexible (default off)</p>
              </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-bold">Edit Schedule - {selectedStaff.name}</h3>
              <button
            ...
            <div className="space-y-6">
                ...
                <div key={day.key} className="border border-gray-200 rounded-lg p-4">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center space-x-4">
                      <input
                        type="checkbox"
                        checked={editingSchedule[day.key as keyof WorkSchedule]?.working}
                        onChange={(e) => updateScheduleDay(day.key, 'working', e.target.checked)}
                  ...
                  {editingSchedule[day.key as keyof WorkSchedule]?.working && (
                    <div className="grid grid-cols-2 gap-4 ml-8">
                      <div>
                        <label className="block text-sm text-gray-600 mb-1">Start Time</label>
                        <input
                          type="time"
                          value={editingSchedule[day.key as keyof WorkSchedule]?.start || '08:00'}
                      ...
                      <div>
                        ...
                        <input
                          type="time"
                          value={editingSchedule[day.key as keyof WorkSchedule]?.end || '17:00'}
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <h4 className="font-medium text-blue-800 mb-2">Schedule Notes:</h4>
                <ul className="text-sm text-blue-700 space-y-1">
                  <li>• Monday-Friday: Default 8am - 5pm</li>
                  <li>• Saturday: Default 8am - 2pm (half day)</li>
                  <li>• Sunday: Flexible - can be scheduled as needed</li>
                  <li>• Check "working" to enable the day, uncheck to mark as day off</li>
                </ul>
              </div>
...
// // app/admin/staffscheduler/page.tsx
// 'use client';
