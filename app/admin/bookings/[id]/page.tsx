// app/admin/bookings/[id]/page.tsx - COMPLETELY FIXED VERSION
'use client';
...
interface Booking {
  ...
  checkOut: string;
  guests: number;
  totalAmount?: number; // Make it optional
  serviceFee?: number; // Make it optional
  priceBreakdown?: PriceBreakdown; // Add priceBreakdown
...
// Helper functions
const getTotalAmount = (booking: Booking | null): number => {
  if (!booking) return 0;
  return booking.priceBreakdown?.totalAmount || booking.totalAmount || 0;
};

const getServiceFee = (booking: Booking | null): number => {
  if (!booking) return 0;
  return booking.priceBreakdown?.serviceCharge || booking.serviceFee || 0;
};
...
export default function BookingDetailsPage() {
  ...
  // Now we can safely use booking since we've checked it exists
  const totalAmount = getTotalAmount(booking);
  const serviceFee = getServiceFee(booking);
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-xl font-bold">₦{booking.onsitePaymentDetails.expectedAmount?.toLocaleString() || totalAmount.toLocaleString()}</p>
                  </div>
                  <div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between border-t border-gray-200 pt-3">
                ...
                <span className="text-[#383a3c] font-bold">₦{totalAmount.toLocaleString()}</span>
              </div>
              <div className="flex justify-between">
