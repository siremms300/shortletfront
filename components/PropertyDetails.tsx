...
export default function PropertyDetails({ property }: PropertyDetailsProps) {
  ...
  // Get price breakdown from property or calculate it
  const priceBreakdown = property.priceBreakdown || 
    calculatePriceBreakdown(
      property.price,
      property.utilityPercentage || 20,
      property.serviceChargePercentage || 10,
      property.vatPercentage || 7.5
    );

  // Check if discount is active
  const hasDiscount = property.discount?.isActive && 
                     (!property.discount.startDate || new Date(property.discount.startDate) <= new Date()) &&
                     (!property.discount.endDate || new Date(property.discount.endDate) >= new Date());

  // Calculate total for multiple nights
  const calculateTotalForNights = (nights: number) => {
    // Use priceBreakdown from property
    if (property.priceBreakdown) {
      return {
        accommodation: property.priceBreakdown.accommodation * nights,
        utility: property.priceBreakdown.utility * nights,
        serviceCharge: property.priceBreakdown.serviceCharge * nights,
        vat: property.priceBreakdown.vat * nights,
        total: property.priceBreakdown.total * nights
      };
    }
    ...
    return {
      accommodation: breakdown.accommodation * nights,
      utility: breakdown.utility * nights,
      serviceCharge: breakdown.serviceCharge * nights,
      vat: breakdown.vat * nights,
      total: breakdown.total * nights
    };
  };
  ...
  // Calculate total prices for the stay
  const totalPrices = totalNights > 0 ? calculateTotalForNights(totalNights) : null;
              ...
              <div className="absolute top-4 left-4 bg-red-500 text-white px-4 py-2 rounded-lg shadow-lg z-10">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-sm font-bold">
                    {property.discount.type === 'percentage' 
                      ? `${property.discount.value}% OFF` 
                      : `₦${property.discount.value.toLocaleString()} OFF`}
                  </span>
                </div>
              </div>
            )}
                ...
                >
                  {loading ? (
                    ...
                    <>
                      ...
                      <span>
                        {hasDiscount 
                          ? `₦${priceBreakdown.actualPrice.toLocaleString()}/night` 
                          : `₦${priceBreakdown.actualPrice.toLocaleString()}/night`}
                      </span>
                      <svg 
            ...
            {property.amenities && property.amenities.length > 0 && (
              <div className="border-b border-gray-200 pb-6">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-xl font-semibold text-[#383a3c]">What this place offers</h3>
                </div>
              ...
              {hasDiscount && property.discount ? (
                <>
                  <div className="flex items-center gap-3 flex-wrap">
                    <span className="text-3xl font-bold text-[#f06123]">
                      ₦{priceBreakdown.actualPrice.toLocaleString()}
                    </span>
                    ...
                    <span className="bg-green-100 text-green-800 px-3 py-1 rounded-full text-sm font-bold">
                      {property.discount.type === 'percentage' 
                        ...
                        : `-₦${property.discount.value.toLocaleString()}`}
                    </span>
                  </div>
                  <p className="text-sm text-gray-500 mt-1">
                    Discounted price + VAT: ₦{priceBreakdown.vat.toLocaleString()}
                  </p>
                  {property.discount.startDate && property.discount.endDate && (
                    <p className="text-xs text-green-600 mt-1">
                      Valid {new Date(property.discount.startDate).toLocaleDateString()} - {new Date(property.discount.endDate).toLocaleDateString()}
                    </p>
                  )}
                </>
              ) : (
                <>
                  <span className="text-3xl font-bold text-[#383a3c]">
                    ₦{priceBreakdown.actualPrice.toLocaleString()}
                  </span>
                  <span className="text-gray-600 text-lg"> / night</span>
                  <p className="text-sm text-gray-500 mt-1">
                    Price + VAT: ₦{priceBreakdown.vat.toLocaleString()}
                  </p>
                </>
              )}
            </div>
                ...
                {totalNights > 0 && totalPrices && (
                  <div className="border-t border-gray-200 pt-4 space-y-3">
                    ...
                    <div className="flex justify-between text-gray-700 text-sm">
                      <span className="text-gray-600">Utility ({property.utilityPercentage || 20}%)</span>
                      <span>₦{totalPrices.utility.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between text-gray-700 text-sm">
                      <span className="text-gray-600">Service Charge ({property.serviceChargePercentage || 10}%)</span>
                      <span>₦{totalPrices.serviceCharge.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between text-gray-700">
                      <span>Subtotal</span>
                      <span>₦{((hasDiscount ? priceBreakdown.actualPrice : property.price) * totalNights).toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between text-gray-700">
                      <span>VAT ({property.vatPercentage || 7.5}% on Accommodation)</span>
                      <span>₦{totalPrices.vat.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between font-bold text-lg border-t border-gray-200 pt-3">
                      <span>Total</span>
                      <span>₦{totalPrices.total.toLocaleString()}</span>
                    </div>
                    ...
                    {hasDiscount && property.discount && (
                      <div className="bg-green-50 p-3 rounded-lg mt-2">
                        <p className="text-green-700 text-sm font-medium">
                          You save: ₦{((property.price - priceBreakdown.actualPrice) * totalNights).toLocaleString()}
                          {property.discount.type === 'percentage' && ` (${property.discount.value}% off)`}
                        </p>
                      </div>
                    )}
                  </div>
...
//   // Get price breakdown from property or calculate it
//   const priceBreakdown = property.calculatedPrices || 
//     calculatePriceBreakdown(
//       property.price,
//       property.utilityPercentage || 20,
//       property.serviceChargePercentage || 10,
...
//   // Calculate total for multiple nights
//   const calculateTotalForNights = (nights: number) => {
//     // Use calculatedPrices from property
//     if (property.calculatedPrices) {
//       return {
//         accommodation: property.calculatedPrices.accommodation * nights,
//         utility: property.calculatedPrices.utility * nights,
//         serviceCharge: property.calculatedPrices.serviceCharge * nights,
//         vat: property.calculatedPrices.vat * nights,
//         total: property.calculatedPrices.total * nights
//       };
//     }
...
//             {/* Breakfast Badge - positioned on image but not floating */}
//             <div className="absolute top-4 right-4 bg-black/80 text-white px-4 py-2 rounded-full backdrop-blur-sm z-10">
//               <div className="flex items-center space-x-2">
//                 <span className="text-lg">🍳</span>
//                 <span className="text-sm font-medium">Complementary Breakfast For One</span>
//               </div>
//             </div>
...
//                       <span>Reserve Now</span>
//                       <span className="text-xl">•</span>
//                       <span>₦{priceBreakdown.total.toLocaleString()}/night</span>
//                       <svg 
//                         className="w-5 h-5 ml-1 animate-bounce" 
//                         fill="none" 
//                         stroke="currentColor" 
...
//         <div className="lg:col-span-1" ref={bookingCardRef}>
//           <div className="sticky top-24 border border-gray-200 rounded-2xl p-6 shadow-xl bg-white">
//             {/* Breakfast Benefit Card - positioned at top of booking card */}
...
//                 </div>
//                 <div>
//                   <h4 className="font-bold text-amber-900 text-sm">COMPLIMENTARY BREAKFAST</h4>
//                   <p className="text-amber-700 text-xs mt-1">Every booking includes breakfast for one person</p>
//                 </div>
//               </div>
//             </div> */}

//             <div className="mb-6">
//               <span className="text-3xl font-bold text-[#383a3c]">₦{priceBreakdown.total.toLocaleString()}</span>
//               <span className="text-gray-600 text-lg"> / night</span>
//               <p className="text-sm text-gray-500 mt-1">
//                 Actual price: ₦{property.price.toLocaleString()} + VAT: ₦{priceBreakdown.vat.toLocaleString()}
//               </p>
//             </div>
...
// //   // const calculateTotalForNights = (nights: number) => {
// //   //   return {
// //   //     accommodation: priceBreakdown.accommodation * nights,
// //   //     utility: priceBreakdown.utility * nights,
// //   //     serviceCharge: priceBreakdown.serviceCharge * nights,
// //   //     vat: priceBreakdown.vat * nights,
// //   //     total: priceBreakdown.total * nights
// //   //   };
// //   // };
...
// //       const actualPrice = property.price * nights;
// //       const utility = (actualPrice * (property.utilityPercentage || 20)) / 100;
// //       const serviceCharge = (actualPrice * (property.serviceChargePercentage || 10)) / 100;
// //       const accommodation = actualPrice - utility - serviceCharge;
// //       const vat = (accommodation * (property.vatPercentage || 7.5)) / 100;
// //       const total = actualPrice + vat;
...
// //     // Use price breakdown from property
// //     return {
// //       accommodation: property.priceBreakdown.accommodation * nights,
// //       utility: property.priceBreakdown.utility * nights,
// //       serviceCharge: property.priceBreakdown.serviceCharge * nights,
...
// //               <div className="flex items-center space-x-2">
// //                 <span className="text-lg">🍳</span>
// //                 <span className="text-sm font-medium">Free Breakfast Included</span>
// //               </div>
// //             </div>
